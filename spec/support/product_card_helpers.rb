...
module ProductCardHelpers
  ...
  def expect_product_cards_in_order(products)
    ...
    products.each_with_index { |product, index| expect(page).to have_selector("article:nth-of-type(#{index + 1})", text: product.name) }
  end

  def expect_product_cards_with_names(*product_names)
    expect(page).to have_product_card(count: product_names.length)
    product_names.each_with_index { |product_name, index| expect(page).to have_selector("article", text: product_name) }
  end
end
