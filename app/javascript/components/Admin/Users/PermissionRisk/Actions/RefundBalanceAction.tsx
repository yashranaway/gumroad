...
const RefundBalanceAction = ({ user }: RefundBalanceActionProps) =>
  ...
  user.unpaid_balance_cents > 0 && (
    <AdminAction
      ...
      url={Routes.refund_balance_admin_user_path(user.external_id)}
      confirm_message={`Are you sure you want to refund user ${user.external_id}'s not paid out purchases!?`}
      done="Refunded."
